...
def draw_confusion_matrix(file_path, num_classes=None, mapping=None, 
                          figure_name='confmat', show_counts=False,
                          class_names=None, figsize=(9,6), norm_axis=0):
...
def build_matrix(data, num_classes=None, mapping=None):
    ...
    assert mapping is None or len(mapping) == num_classes, (
            "The number of classes should match those in the map.")

    # Build the confusion matrix
    hist = np.histogram2d(
            data.pred, data.label, bins=[num_classes, num_classes],
